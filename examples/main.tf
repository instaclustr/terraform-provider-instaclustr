...
terraform {
  required_providers {
    instaclustr = {
    //Use the instaclustr/instaclustr source to pull the latest release from the terraform registry
     //source = "instaclustr/instaclustr"
    //Use the following (with your version number) to use a locally built copy
    source = "terraform.instaclustr.com/instaclustr/instaclustr"
     version = ">= 1.0.0, < 2.0.0"
    }
  }
...
provider "instaclustr" {
  username = "kartiksharmapreprod"
  api_key = "2a4a317c69a0b874e87971f1f01577a3"
  //comment out the appropriate line based on whether you wish to test in preprod or local
